...
--Code Starts Here
SELECT fname, lname, dob, salary FROM chave_student WHERE ROUND(MONTHS_BETWEEN(SYSDATE,dob)/12) > 15;
...
SELECT fname, lname, NVL(TO_CHAR(dob),'not born yet'), salary 
FROM chave_student 
WHERE LOWER(fname) LIKE 'abr%' AND ROUND(MONTHS_BETWEEN(SYSDATE,dob)/12) BETWEEN 15 AND 25;

SELECT fname, lname, dob, salary, DECODE(SIGN(salary-20000),1,'rich',-1,'poor',0,'rich')
FROM chave_student
WHERE dob IS NOT NULL;

SELECT dob, (SQRT(ROUND(MONTHS_BETWEEN(SYSDATE,dob)/12))+20)/5
FROM chave_student;
...
SELECT fname, lname, dob, salary
FROM chave_student
WHERE LOWER(SUBSTR(fname,1,3)) LIKE '%b' AND salary>10000 AND phone LIKE '527%';

DROP TABLE chave_student2;
CREATE TABLE chave_student2 AS SELECT fname,lname,salary*2 new_salary
FROM chave_student
WHERE lname LIKE '%nn%' AND DOB IS NULL;
SELECT * FROM chave_student2;